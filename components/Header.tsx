    ...
    <div>
      ...
      >
        ...
        right-2 sm:right-10 hover:bg-blue-700  transition-colors duration-300"
          >
            {truncate({ text: wallet, startChars: 4, endChars: 3, maxLength: 11 })}
          </button>
        ) : (
          <button
            className="text-sm bg-blue-600 rounded-full w-[150px] h-[48px] text-white
      ...
      <div className="h-[48px] w-[90%] border border-[#212D4A] mx-auto rounded-full space-x-2 md:hidden px-1 flex items-center">
        <div className="bg-[#1B1631] rounded-full p-3">
          <AiOutlineSearch className="text-[#4555887]" />
        </div>

        <input
          placeholder="Search here"
          className="bg-transparent -mt-1 outline-none text-[14px] placeholder:text-xs placeholder:text-[#BBBBBB]"
        />
      </div>
    </div>
  )
